...
**Log a gym / manual workout.** Tap "✍️ Log a gym / manual workout" on the home screen to record what you did yourself — nothing is preset. Add cardio by the minute (treadmill, elliptical, stair master) or strength moves with your own reps × sets (e.g. back extensions), pick an intensity (Light / Moderate / Intense), and stack several activities into one session. Saving awards XP and coins on the same economy as a guided workout, so it counts toward your streak, the weekly challenge, badges, and the squad leaderboard. Manual sessions appear in Recent activity tagged "✍️ logged" and can be deleted (rolling back their XP/coins) like any workout.
